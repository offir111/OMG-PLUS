...
export default function FaithChatPanel() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (atBottom) setUnreadFab(0);
    };
    el.addEventListener('scroll', onScroll, { passive: true });
    return () => el.removeEventListener('scroll', onScroll);
  }, [joinedLobby, activePanel]);
  ...
  const sendPublic = useCallback(() => {
    ...
    const reply = replyTarget ? { id: replyTarget.id, displayName: replyTarget.displayName, text: replyTarget.text } : undefined;
    setPublicMessages(prev => [
      ...prev.slice(-300),
      {
  ...
  }, [draftPublic, joinedLobby, mySocketId, outgoingColor, replyTarget]);
  ...
  return (
    <>
      <style>{`
        ...
        .cage-tool-btn:hover { background: rgba(99,102,241,0.15); color: #a5b4fc; }
        .cage-tool-btn.on { background: rgba(99,102,241,0.2); color: #a5b4fc; border-color: rgba(99,102,241,0.4); }
        .cage-tool-btn svg { display: block; overflow: visible; flex-shrink: 0; }
        .cage-input-wrap {
          flex: 1;
          min-width: 0;
          direction: rtl;
        ...
        .cage-input-wrap textarea {
          ...
          padding: 8px 14px;
          text-align: right;
          font-family: "Heebo", var(--font-sans, Rubik, system-ui, sans-serif);
          color: var(--text);
          box-shadow: none;
          line-height: 1.4;
          transition: border-color 0.2s, background 0.2s;
        }
        .cage-input-wrap textarea::placeholder { color: rgba(100,116,139,0.8); }
        .cage-input-wrap textarea:focus {
          outline: none;
        ...
        .cage-send-cage:hover:not(:disabled) {
          background: #818cf8;
          box-shadow: 0 4px 18px rgba(129,140,248,0.6);
          transform: scale(1.05);
        }
        .cage-send-cage:active:not(:disabled) { transform: scale(0.95); }
        .cage-send-cage:disabled { opacity: 0.35; cursor: not-allowed; background: #3730a3; box-shadow: none; }
        .cage-footer-tabs {
          display: flex;
          flex-wrap: wrap;
          gap: 4px;
        ...
        @keyframes fcpMsgAppear {
          from { opacity: 0; transform: translateY(7px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @media (prefers-reduced-motion: reduce) {
          .fcp-msg-article { animation: none !important; }
        }
        .fcp-msg-list {
          flex: 1;
          overflow-y: auto;
        ...
        /* ── Message article wrapper ── */
        .fcp-msg-article {
          position: relative;
          display: flex;
          flex-direction: column;
          align-items: flex-start;
          animation: fcpMsgAppear 0.18s ease-out both;
          margin-bottom: 2px;
        }
        .fcp-msg-article.mine {
          align-items: flex-end;
        }
        .fcp-msg-article--grouped {
          margin-bottom: 1px;
        }
        /* ── Avatar row ── */
        .fcp-msg-avatar-row {
        ...
        .fcp-msg-article.mine .fcp-msg-avatar-row {
          flex-direction: row-reverse;
        }
        .fcp-msg-avatar-placeholder {
          width: 30px;
          flex-shrink: 0;
        }
        /* ── Bubble ── */
        ...
        .fcp-msg-article--grouped .fcp-bubble {
          border-radius: 18px;
        }
        .fcp-msg-article--grouped .fcp-bubble--mine {
          border-radius: 18px;
        }
        ...
        .fcp-bubble--mine .fcp-bubble-reply {
        ...
        .fcp-msg-article:focus-within .fcp-mod-delete { opacity: 1; }
        .fcp-mod-delete:hover { background: rgba(244,63,94,0.65); color: #fff; }
        @media (max-width: 820px) { .fcp-mod-delete { opacity: 0.6; } }
        /* ── Typing indicators ── */
        .fcp-typing {
          display: flex;
          align-items: center;
        ...
        .fcp-typing-dots span {
          ...
          border-radius: 50%;
          background: #818cf8;
          animation: fcpTypingDot 1.2s ease-in-out infinite;
          opacity: 0.4;
        }
        .fcp-typing-dots span:nth-child(1) { animation-delay: 0s; }
        .fcp-typing-dots span:nth-child(2) { animation-delay: 0.18s; }
        .fcp-typing-dots span:nth-child(3) { animation-delay: 0.36s; }
        @keyframes fcpTypingDot {
          0%, 100% { opacity: 0.3; transform: translateY(0); }
          45% { opacity: 1; transform: translateY(-4px); }
        }
        @media (prefers-reduced-motion: reduce) {
          .fcp-typing-dots span { animation: none !important; opacity: 0.85; }
        }
        /* ── Jump-to-bottom FAB ── */
        .fcp-scroll-fab {
          position: absolute;
        ...
        .fcp-scroll-fab:hover { background: rgba(30,27,75,0.96); border-color: rgba(129,140,248,0.6); }
        .fcp-scroll-fab-badge {
          background: #818cf8;
          color: #fff;
          font-size: 0.6rem;
          font-weight: 800;
        ...
        .fcp-conn-banner-dot {
          ...
          background: #fbbf24;
          animation: fcpConnPulse 1.4s ease-in-out infinite;
        }
        @keyframes fcpConnPulse {
        ...
        .fcp-reply-strip-x:hover { background: rgba(244,63,94,0.28); color: #fca5a5; }
        .fcp-compose {
          display: flex;
          gap: 8px;
          padding-top: 6px;
        ...
        .fcp-send:hover:not(:disabled) { background: #818cf8; transform: scale(1.05); }
        .fcp-send:active:not(:disabled) { transform: scale(0.95); }
        .fcp-send:disabled { opacity: 0.35; cursor: not-allowed; background: #3730a3; box-shadow: none; }
        .fcp-compose-gear {
          flex-shrink: 0;
          order: 99;
          display: inline-flex;
        ...
        .fcp-connect-btn {
          ...
          box-shadow: 0 4px 24px rgba(16,185,129,0.35), inset 0 1px 0 rgba(255,255,255,0.18);
        ...
        .fcp-connect-btn:active:not(:disabled) { transform: translateY(0); }
            ...
            <div className="fcp-grid fcp-grid--solo">
              <div className="fcp-main cage-shell">
                ...
                <div
                  ...
                  ref={listScrollRef}
                  style={{ flex: 1, minHeight: 200, maxHeight: 'min(54vh, 480px)', overflowY: 'auto' }}
                >
                  {showScrollFab && (
                    <button
                      type="button"
                      className="fcp-scroll-fab"
                      onClick={() => {
                        const el = listScrollRef.current;
                    ...
                    >
                      ↓ גלול לתחתית
                      {unreadFab > 0 && (
                        <span className="fcp-scroll-fab-badge">{unreadFab > 99 ? '99+' : unreadFab}</span>
                      )}
                    </button>
                  )}
                  {activePanel === 'public' ? (
                    generalSubView === 'presence' ? (
                      ...
                      <div className="fcp-placeholder">
                        אין עדיין תוכן בצ&apos;אט הציבורי. שלחו הודעה — או עברו ל«נוכחים» ופתחו שיחה פרטית.
                      </div>
                    ) : (
                      <>
                        {(() => {
                          let lastDay = null;
                          return publicConversationList.map((m, idx) => {
                            if (m.kind === 'system') {
                              return (
                            ...
                            const prev = publicConversationList[idx - 1];
                            const sameAsPrev =
                              prev &&
                              prev.kind !== 'system' &&
                              prev.fromSocketId === m.fromSocketId &&
                              m.ts - (prev.ts || 0) < 90000;
                            const dayStr = m.ts
                              ? new Date(m.ts).toLocaleDateString('he-IL', { weekday: 'long', day: 'numeric', month: 'long' })
                              : null;
                            const showDayPill = dayStr && dayStr !== lastDay;
                            if (showDayPill) lastDay = dayStr;
                            const timeStr = prefs.showTimestamps && m.ts
                              ? new Date(m.ts).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })
                              : null;
                            return (
                              <React.Fragment key={m.id}>
                                {showDayPill && (
                                  <div className="fcp-day-pill" aria-label={dayStr}>{dayStr}</div>
                                )}
                                <article
                                  className={`fcp-msg-article${mine ? ' mine' : ''}${sameAsPrev ? ' fcp-msg-article--grouped' : ''}`}
                                >
                                  <div className="fcp-msg-avatar-row">
                                    {!mine && !sameAsPrev && (
                                      <UserAvatarSlot
                                        size="chat"
                                    ...
                                    {!mine && sameAsPrev && <div className="fcp-msg-avatar-placeholder" />}
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: mine ? 'flex-end' : 'flex-start', maxWidth: '72%' }}>
                                      {!sameAsPrev && (
                                        <span className={`fcp-bubble-name${mine ? ' fcp-bubble-name--mine' : ''}`}>
                                          {m.displayName}
                                        </span>
                                      )}
                                      <div
                                        className={`fcp-bubble${mine ? ' fcp-bubble--mine' : ''}`}
                                        style={m.color && !mine ? { color: m.color } : undefined}
                                        onDoubleClick={() => setReplyTarget({ id: m.id, displayName: m.displayName, text: m.text })}
                                        title="לחיצה כפולה למענה"
                                      >
                                        {m.replyTo && (
                                          <div className="fcp-bubble-reply">
                                            <strong>{m.replyTo.displayName}:</strong> {m.replyTo.text?.slice(0, 80)}
                                          </div>
                                        )}
                                        {m.text}
                                        {timeStr && <div className="fcp-bubble-time">{timeStr}</div>}
                                      </div>
                                    </div>
                                    {mine && !sameAsPrev && (
                                      <UserAvatarSlot
                                  ...
                                  {faithChatModerator && (
                                    <button
                                      ...
                                      aria-label={`מחיקת הודעת ציבור (מנהל): ${m.displayName}`}
                                      title="מחיקה מהציבור לכולם (מנהל)"
                                      onClick={e => { e.preventDefault(); deletePublicMessageAsModerator(m.id, `${m.displayName}: ${m.text}`); }}
                                    >×</button>
                                  )}
                                </article>
                              </React.Fragment>
                        ...
                        {showOritTypingDemo && (
                          <div className="fcp-public-typing-demo" aria-live="polite">
                            ...
                            />
                            <div className="fcp-bubble" style={{ padding: '8px 14px' }}>
                              <span className="fcp-public-typing-label">אורית כותבת</span>
                    ...
                    <>
                      {activePrivateSlot.messages.length === 0 ? (
                        <div className="fcp-placeholder">
                          שיחה פרטית עם {activePrivateSlot.partner.name}. כתבו הודעה למטה להתחלת השיחה.
                        </div>
                      ) : (
                        (() => {
                          let lastDay = null;
                          return activePrivateSlot.messages.map((m, idx) => {
                            const mine = m.fromSocketId === sid;
                            const prev = activePrivateSlot.messages[idx - 1];
                            const sameAsPrev =
                              prev &&
                              prev.fromSocketId === m.fromSocketId &&
                              m.ts - (prev.ts || 0) < 90000;
                            ...
                            return (
                              <React.Fragment key={m.id}>
                                {showDayPill && (
                                  <div className="fcp-day-pill">{dayStr}</div>
                                )}
                                <article
                                  className={`fcp-msg-article${mine ? ' mine' : ''}${sameAsPrev ? ' fcp-msg-article--grouped' : ''}`}
                                >
                                  <div className="fcp-msg-avatar-row">
                                    {!mine && !sameAsPrev && (
                                      <UserAvatarSlot size="chat" displayName={m.displayName} avatarUrl={resolveFaithChatLineAvatarUrl(m)} />
                                    )}
                                    {!mine && sameAsPrev && <div className="fcp-msg-avatar-placeholder" />}
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: mine ? 'flex-end' : 'flex-start', maxWidth: '72%' }}>
                                      {!sameAsPrev && (
                                        <span className={`fcp-bubble-name${mine ? ' fcp-bubble-name--mine' : ''}`}>{m.displayName}</span>
                                      )}
                                      <div
                                        className={`fcp-bubble${mine ? ' fcp-bubble--mine' : ''}`}
                                        style={m.color && !mine ? { color: m.color } : undefined}
                                  ...
                                  {faithChatModerator && dmModerationRoomId && (
                                    <button
                                      ...
                                      onClick={e => { e.preventDefault(); deleteDmMessageAsModerator(m.id, `${m.displayName}: ${m.text}`); }}
                                    >×</button>
                                  )}
                                </article>
                              </React.Fragment>
                      ...
                      {dmChannelReady &&
                        ...
                        <div className="fcp-typing" aria-live="polite">
                          <div className="fcp-bubble" style={{ padding: '7px 12px' }}>
                            <span className="fcp-public-typing-label">{activePrivateSlot.partner.name} כותב/ת</span>
                            <span className="fcp-typing-dots" aria-hidden="true"><span /><span /><span /></span>
                          </div>
                        </div>
                      )}
                    </>
                ...
                {joinedLobby && !chatConnected && (
                  <div className="fcp-conn-banner" role="alert">
                    <span className="fcp-conn-banner-dot" aria-hidden />
                    החיבור לשרת נותק — מנסה להתחבר מחדש…
                  </div>
                )}

                {replyTarget && (
                  <div className="fcp-reply-strip">
                    <span className="fcp-reply-strip-quote">
                      <span className="fcp-reply-strip-name">{replyTarget.displayName}: </span>
                      {replyTarget.text?.slice(0, 80)}
                    </span>
                    <button
                      type="button"
                      className="fcp-reply-strip-x"
                    ...
                    {activePanel === 'public' ? (
                      chatConnected ? (
                        <>
                          ...
                          <button
                            ...
                            onClick={sendPublic}
                            disabled={!chatConnected || !draftPublic.trim()}
                            aria-label="שלח הודעה"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>
                          </button>
                        </>
                      ) : (
                      ...
                      <>
                        ...
                        <button
                          ...
                          title={!activePrivateSlot?.roomReady ? 'ממתין לחיבור השיחה — רגע אחד' : 'שלח הודעה'}
                          aria-label="שלח הודעה"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>
                        </button>
