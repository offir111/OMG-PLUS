  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setHomeLivePlayingKey(prev => {
        ...
        const ent = mergedLiveBroadcastRef.current['faith-1'];
        if (!homeLiveEntryHasListenSource(ent)) return null;
        return 'faith-1';
      });
    }, 450);
    ...
    <div ref={panelRef} className="header-podcast-panel" style={{ top: topOffset }} role="region" aria-label="פודקאסט LIVE">
      <div className="header-podcast-panel__inner">
        <div className="home-live-broadcast-block">
          ...
          <div className="home-live-broadcast-grid" dir="rtl" aria-label="פודקאסטים — הקשבה">
            {/* כותרת + X */}
            <div className="home-live-grid-header">
              <span className="home-live-broadcast-label">פודקאסט LIVE</span>
              {isOmgLiveEditor ? (
              ...
              {[1, 2, 3].map(slot => {
                ...
                return (
                  <button key={tabKey} type="button" role="listitem"
                    ...
                    {homeLivePlayingKey === tabKey ? '⏸ ' : '▶ '}{btnLabel}
                  </button>
                );
              })}
            </div>
