          ...
          <div className="mini-radio-bar__inner">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
              ...
              {playerMode === 'radio' ? (
                <>
                  <select
                    id="mini-media-bar-radio"
                    className="mini-radio-bar__select"
                    value={stationId ?? ''}
                    onChange={e => setStationId?.(e.target.value, { fromUserPick: true })}
                    aria-label="תחנת רדיו"
                  >
                ...
                <>
                  <select
                    id="mini-media-bar-youtube"
                    className="mini-radio-bar__select"
                    value={ytSelectedId}
                    onChange={e => { const n = Number(e.target.value); setYtSelectedId(n); writeYtSelectedStationId(n); setYtTvUrl(null); }}
                    aria-label="תחנת יוטיוב"
                  >
                    {ytStations.map(s => <option key={s.id} value={s.id}>{s.id}. {s.name}</option>)}
                  </select>
                  <button type="button" className="mini-radio-bar__btn" onClick={toggleYoutubePlayback}
                  ...
                  <button type="button" className="mini-radio-bar__btn mini-yt__gear" onClick={openConfig}
                    title="ערוך תחנות יוטיוב" aria-label="ערוך תחנות יוטיוב">⚙</button>
                </>
              )}
            </div>

            {/* שורה תחתונה: ווליום מימין לשמאל */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', direction: 'rtl' }}>
              <span style={{ fontSize: '1rem', flexShrink: 0 }}>🔊</span>
              <input
                type="range"
                className="mini-radio-bar__vol--shared"
                style={{ flex: 1, accentColor: 'var(--gold)', cursor: 'pointer', height: 6, direction: 'rtl' }}
                min={0} max={1} step={0.01}
                value={sharedVol}
                onChange={e => handleVol(Number(e.target.value))}
                aria-label="עוצמת שמע"
              />
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-secondary)', flexShrink: 0, minWidth: 28, textAlign: 'center' }}>
                {Math.round(sharedVol * 100)}%
              </span>
            </div>
