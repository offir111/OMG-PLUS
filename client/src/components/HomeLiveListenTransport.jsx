...
export default function HomeLiveListenTransport({ tabKey, directUrl, youtubeVideoId = '' }) {
  ...
  const handleVolume = useCallback(val => {
    setVolume(val);
    if (direct && audioRef.current) {
      audioRef.current.volume = val;
    } else if (youtubeOnly && ytPlayerRef.current?.setVolume) {
      try { ytPlayerRef.current.setVolume(Math.round(val * 100)); } catch { /* ignore */ }
    }
  }, [direct, youtubeOnly]);
  ...
  return (
    <div className={`home-live-transport home-live-transport--slim${youtubeOnly ? ' home-live-transport--youtube-audio' : ''}`} dir="ltr">
      ...
      {/* פיידר ווליום */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, direction: 'rtl', padding: '6px 0 2px' }}>
        ...
        <input
          ...
          style={{ flex: 1, accentColor: '#ef4444', cursor: 'pointer', height: 6, direction: 'rtl' }}
        />
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'rgba(255,255,255,0.7)', flexShrink: 0, minWidth: 32, textAlign: 'center' }}>
