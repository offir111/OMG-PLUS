...
import VirtualUserProfile from '../components/profile/VirtualUserProfile.jsx';
import {
        ...
        <div style={{ paddingBottom: 4 }}>
          ...
          {username && (
            <div style={{ display: 'flex', gap: 8, paddingTop: 8, alignItems: 'flex-end' }}>
              ...
              />
              <button type="button" onClick={handleComment} disabled={!draft.trim()} style={{ padding: '10px 15px', minHeight: 44, borderRadius: 6, border: '1px solid rgba(99,102,241,0.4)', background: draft.trim() ? 'rgba(99,102,241,0.85)' : 'var(--card-hover)', color: '#fff', fontWeight: 800, fontSize: '0.88rem', cursor: draft.trim() ? 'pointer' : 'not-allowed', flexShrink: 0, touchAction: 'manipulation' }}>
                שלח
              </button>
            </div>
          )}
...
export default function CageUserProfilePage() {
  ...
  /** פרופיל וירטואל מהשרת — אם null אז לא וירטואל */
  ...
  // בדוק אם זה פרופיל וירטואל — שלוף מהשרת
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${BASE}/api/virtual-users/${enc}`).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`${BASE}/api/virtual-feed/user/${enc}`).then(r => r.ok ? r.json() : null).catch(() => null),
    ]).then(([profile, feed]) => {
      if (cancelled) return;
      setVirtualData(profile?.isVirtualUser ? profile : null);
      setVirtualPosts(Array.isArray(feed?.posts) ? feed.posts : []);
  ...
  // ── Loading skeleton while checking if virtual ────────────────────────
  if (virtualLoading) {
    return (
      <div className="cage-prof-root" style={{
        ...
        <span style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>טוען פרופיל…</span>
      </div>
    );
  }
    ...
    >
      ...
      {msgOpen && viewerNorm ? (
        ...
        <div style={{ direction: 'rtl', display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 160px)', background: 'var(--bg)' }}>
          ...
          <div style={{
            ...
            <button
              ...
              style={{ width: 40, height: 40, borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.65)', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'manipulation', flexShrink: 0 }}
            >✕</button>
          </div>
        ...
        <div style={{ direction: 'rtl', display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 160px)', background: 'var(--bg)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface)', padding: '9px 14px', borderBottom: '1px solid var(--border-strong)' }}>
            ...
            <button type="button" onClick={() => setGalleryOpen(false)} style={{ width: 40, height: 40, borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.65)', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'manipulation', flexShrink: 0 }}>✕</button>
          </div>
                    ...
                    {isOwner && (
                      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex', gap: 3, padding: 4, background: 'rgba(0,0,0,0.55)' }}>
                        <button
                          ...
                          style={{ flex: 1, padding: '5px 2px', borderRadius: 4, fontSize: '0.72rem', fontWeight: 800, border: 'none', cursor: 'pointer', background: item.isPublic ? 'rgba(168,85,247,0.85)' : 'rgba(255,255,255,0.15)', color: '#fff', touchAction: 'manipulation' }}
                        >
                        ...
                        <button
                          ...
                          style={{ width: 30, borderRadius: 4, fontSize: '0.78rem', fontWeight: 800, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.75)', color: '#fff', touchAction: 'manipulation' }}
                        >
                          ✕
                        </button>
                      </div>
      ...
      <section style={{ marginTop: 2 }}>
        ...
        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {canEditProfile && viewerNorm === 'omg' ? (
            <button
              type="button"
              className="cage-blog-btn"
              onClick={() => navigate('/admin')}
              style={{ background: 'linear-gradient(135deg, rgba(234,179,8,0.9), rgba(161,122,0,0.85))', color: '#000', fontWeight: 900 }}
            >
              👑 פאנל ניהול
            </button>
          ) : null}
